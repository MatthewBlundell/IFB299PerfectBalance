...
# Monthly data placed into month list.
        ...
        for z in Order.objects.filter(pickupdate=date):
            ordersmonth.append(z)

# Monthly data broken down into weekly segments.
# This is used in the Monthly rental chart.
    week1total = sum(monthcounts[0:7])
    week2total = sum(monthcounts[7:14])
    week3total = sum(monthcounts[14:21])
    week4total = sum(monthcounts[21:28])
...
# prepares the data for displaying in a styled table in the HTML.
    ...
    for i in ordersmonth:
        vehicles.append(Vehicle.objects.get(carid=str(i.carid)))
